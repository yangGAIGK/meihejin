<template>
  <div class="container">
    ...
    >
      ...
      <!-- 表格列 -->
      <el-table-column prop="maxNum" label="最大数值" min-width="150" />
      <el-table-column prop="topValue" label="上界值" min-width="150" />
      <el-table-column prop="lowValue" label="下界值" min-width="150" />
      <el-table-column prop="swarmSize" label="种群规模" min-width="150" />
      <el-table-column prop="individualFactor" label="个体因子" min-width="150" />
      <el-table-column prop="groupFactor" label="群体因子" min-width="150" />
      <el-table-column prop="inertiaFactor" label="惯性因子" min-width="150" />
  ...
  methods: {
    ...
    async fetchHistoryData() {
      ...
      try {
        ...
        if (response.data.code === 1) {
          // 移除 createUser 和 id 字段
          this.table = response.data.data.records.map(record => {
            const { createUser, id, ...rest } = record;
            return {
              ...rest,
              createTime: record.createTime ? record.createTime.replace("T", " ") : null,
              updateTime: record.updateTime ? record.updateTime.replace("T", " ") : null,
            };
          }) || [];
    ...
    async apply(row) {
      ...
      try {
        // 调用 API 并传递数据
        const response = await axios.post(
            "http://127.0.0.1:5000/psopredict",
            requestData
        );
    ...
    async sendDataToBackend(data) {
      try {
        ...
        const response = await axios.post(
            "http://localhost:8080/psoparameter",
            data,
            {
              headers: {
                Authorization: `Bearer ${token}`, // 将 token 添加到请求头
...
.container {
  ...
  @media (max-width: 768px) {
    ...
    .title {
      font-size: 24px; // Adjusted title size for smaller screens
    }
  }
}
</style>
