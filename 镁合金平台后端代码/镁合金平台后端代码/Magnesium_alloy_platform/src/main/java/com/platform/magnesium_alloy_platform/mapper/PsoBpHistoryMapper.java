...
@Mapper
public interface PsoBpHistoryMapper {

    @Select("SELECT * FROM psobp_history WHERE uid = #{uid}")
    @Results({
    ...
    Page<PsoBpHistory> findHistory(PsoHistoryDTO psoHistoryDTO);

    @Insert("INSERT INTO psobp_history (aveLength, lisanValue, hardness, yield_strength, strength_extension, " +
    ...
    void add(PsoBpHistory psobpHistory);
}
