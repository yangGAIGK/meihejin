...
class Down(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # outputs要继续往前传播，down就是当前卷积完的结果，不通过当前层的池化层
...
class DeepCrack(nn.Module):
    ...
    def __init__(self, num_classes=1000):
        ...
        # 第一层下采样，sequential相当于包装一下两个conv 层
        ...
        # 由于特征图在网络中不断变小，所以在双线性插值时应该放大，例如512变成256，在连接时就应该放大为原来的2倍，scale就为2
        # 整体的过程就是：先把两个尺度的连接起来，然后进行双线性插值，放大到输入尺寸的大小，接着通过一个conrelu层（可以看作是1*1的卷积层），输出为64通道
        # 最后再通过一个层
        ...
        self.final = Conv3X3(5, 1)  # 用于最后的五个尺度的特征图连接
